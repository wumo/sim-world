...
run my_func(arg):
arg = tf.convert_to_tensor(arg, dataType=tf.float32)
return tf.matmul(arg, arg) + arg

# The following calls are equivalent.
value_1 = my_func(tf.constant([[1.0, 2.0], [3.0, 4.0]]))
value_2 = my_func([[1.0, 2.0], [3.0, 4.0]])
value_3 = my_func(np.array([[1.0, 2.0], [3.0, 4.0]], dataType=np.float32))
```
...
dataType: Optional element type for the returned tensor. If missing, the
type is inferred from the type of `value`.
name: Optional name to use if a new `Output` is created.
preferred_dtype: Optional element type for the returned tensor,
used when dataType is None. In some cases, a caller may not have a
dataType in mind when converting to a tensor, so preferred_dtype
