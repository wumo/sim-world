...
interface const_ops {
  ...
  fun const(shape: Shape, value: Float, name: String = "Const") = const(shape, DT_FLOAT, name) { add_float_val(value) }
  fun const(shape: Shape, value: Double, name: String = "Const") = const(shape, DT_DOUBLE, name) { add_double_val(value) }
  fun const(shape: Shape, value: Boolean, name: String = "Const") = const(shape, DT_BOOL, name) { add_bool_val(value) }
  fun const(shape: Shape, value: Byte, name: String = "Const") = const(shape, DT_INT8, name) { add_int_val(value.toInt()) }
  fun const(shape: Shape, value: Short, name: String = "Const") = const(shape, DT_INT16, name) { add_int_val(value.toInt()) }
  fun const(shape: Shape, value: Int, name: String = "Const") = const(shape, DT_INT32, name) { add_int_val(value) }
  fun const(shape: Shape, value: Long, name: String = "Const") = const(shape, DT_INT64, name) { add_int64_val(value) }
  fun const(shape: Shape, value: String, name: String = "Const") = const(shape, DT_STRING, name) { add_string_val(value) }
  ...
  fun const(dtype: Int, value: Any, name: String = "Const") =
      const(scalarDimension, dtype, value, name)
  
  fun const(shape: Shape, dtype: Int, value: Any, name: String = "Const"): Output {
    return const_switch(dtype, shape, value, name)
  }
  
  private fun const(shape: Shape, dtype: Int, name: String = "Const", set_value: TensorProto.() -> Unit): Output {
