...
#include "G:/vs_projects/tensorflow/tensorflow/contrib/cmake/build/tensorflow/cc/ops/batch_ops.h"
...
BatchFunction::BatchFunction(const ::tensorflow::Scope& scope,
                             ::tensorflow::InputList in_tensors,
                             ::tensorflow::InputList captured_tensors, const
                             NameAttrList& f, int64 num_batch_threads, int64
                             max_batch_size, int64 batch_timeout_micros, const
                             DataTypeSlice& Tout, const BatchFunction::Attrs&
                             attrs) {
  if (!scope.ok()) return;
  auto _in_tensors = ::tensorflow::ops::AsNodeOutList(scope, in_tensors);
  if (!scope.ok()) return;
  ...
  for (int32 i = 0; i < ret->num_outputs(); ++i)
    this->out_tensors.push_back(Output(ret, i));
}
...
BatchFunction::BatchFunction(const ::tensorflow::Scope& scope,
                             ...
                             max_batch_size, int64 batch_timeout_micros, const
                             DataTypeSlice& Tout)
  : BatchFunction(scope, in_tensors, captured_tensors, f, num_batch_threads, max_batch_size, batch_timeout_micros, Tout, BatchFunction::Attrs()) {}
