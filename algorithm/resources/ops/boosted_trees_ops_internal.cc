...
#include "G:/vs_projects/tensorflow/tensorflow/contrib/cmake/build/tensorflow/cc/ops/boosted_trees_ops_internal.h"
...
BoostedTreesCenterBias::BoostedTreesCenterBias(const ::tensorflow::Scope&
  ...
  if (!scope.ok()) return;
  scope.UpdateStatus(scope.DoShapeInference(ret));
  this->continue_centering = Output(ret, 0);
}
...
BoostedTreesExampleDebugOutputs::BoostedTreesExampleDebugOutputs(const
                                                                 ::tensorflow::Scope&
                                                                 scope,
                                                                 ::tensorflow::Input
                                                                 tree_ensemble_handle,
  ...
  if (!scope.ok()) return;
  scope.UpdateStatus(scope.DoShapeInference(ret));
  this->examples_debug_outputs_serialized = Output(ret, 0);
}
