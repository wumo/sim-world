...
if [[ -z ${OPERATION:-} ]]; then
    echo "Usage: bash cppbuild.sh [-platform <name>] [-extension <name>] <install | clean> [projects]"
    echo "where possible platform names are: linux-x86_64, macosx-x86_64, windows-x86_64, etc."
    exit 1
fi

if [[ -z ${PROJECTS:-} ]]; then
    PROJECTS=(atari-py buf)
fi
